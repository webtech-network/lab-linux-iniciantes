...
Como instalar e configurar a distribuição Linux em sua máquina.
...
Siga o menu de instalação, selecionando as opções conforme as suas preferências.
...
Nas distribuições Linux os programas são distribuídos por meio de pacotes. Para instalar um pacote é necessário utilizar um gerenciador de pacotes. Cada distribuição possuí seu(s) respectivo(s) gerenciador(es) de pacotes. No Ubuntu os gerenciadores de pacotes são: o APT e o Snap, além da loja de aplicativos.
...
```
sudo snap install <nome-do-pacote> --classic
```
